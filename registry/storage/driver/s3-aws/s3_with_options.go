...
type RequestContextCancelledFunc func(driverName string, err error) error
...
func (w *WithOptions) GetContent(ctx context.Context, path string) ([]byte, error) {
	...
	return data, w.checkForContextCancel(w.Name(), err)
}

func (w *WithOptions) PutContent(ctx context.Context, path string, content []byte) error {
	return checkS3RequestContextCancellation(w.Name(), w.s3.PutContent(ctx, path, content))
}
...
func (w *WithOptions) Reader(ctx context.Context, path string, offset int64) (io.ReadCloser, error) {
	...
	return reader, checkS3RequestContextCancellation(w.Name(), err)
}
...
func checkS3RequestContextCancellation(driverName string, err error) error {
	if err != nil {
